...
local Y_OFFSET = -70
local X_OFFSET = 0
local SCROLL_HEIGHT_OFFSET = 0
local SAVE_Y_OFFSET = -5
local SAVE_X_OFFSET = 0
...
esm.eventHandlerFrame:SetScript("OnEvent", function(self, event, arg1)
	if event == "ADDON_LOADED" then
		if arg1 == ADDON_NAME then
			if EssenceSetManager == nil then
				EssenceSetManager = {}
			end
			esm.specProfiles = EssenceSetManager
			esm.createFonts()
		elseif arg1 == "Blizzard_AzeriteEssenceUI" then
			esm.createSaveFrames()
			esm.createScrollFrame()
...
esm.azeriteUIHide = function()
	esm.azeriteUIOpen = false
	esm.setInputText("")
	esm.checkEvents() --checks if LDB menu is open and turns off aura checking and resting checking events if it's not.
end
...
esm.updateTomeButton = function()
	if esm.itemBtn then
		...
		esm.itemBtn:SetText("Use Tome ("..tomeCount..")" )
...
esm.updateSetButtons = function()
	if _G.AzeriteEssenceUI then
		...
		esm.buttonContainer:SetHeight(((BUTTON_SIZE + BUTTON_PAD * 2) * (i-1) + (BUTTON_PAD*2)))
		
		if i < MAX_SETS + 1 then
			esm.scrollFrame.ScrollBar:Hide()
			esm.scrollFrame:SetSize(BUTTON_SIZE+50, AzeriteEssenceUI:GetHeight()+Y_OFFSET-15)
		else
			esm.scrollFrame.ScrollBar:Show()
			esm.scrollFrame:SetSize(BUTTON_SIZE+20, AzeriteEssenceUI:GetHeight()+Y_OFFSET -15 - SCROLL_HEIGHT_OFFSET)
		end
...
esm.createFonts = function()
	local fontStr = "Interface\\AddOns\\EssenceSetManager\\media\\fonts\\DORISBR.TTF"
	local fontSize = 13

	esm.fontWhite = CreateFont("noMatch")
	esm.fontWhite:CopyFontObject("GameFontNormal");
	esm.fontWhite:SetTextColor(1, 1, 1, 1.0);
	esm.fontWhite:SetFont(fontStr, fontSize, "OUTLINE")
	
	esm.fontTome = CreateFont("Tome")
	esm.fontTome:CopyFontObject("GameFontNormal");
	esm.fontTome:SetTextColor(1, 1, 1, 1.0);
	esm.fontTome:SetFont(fontStr, 16, "OUTLINE")
	
	esm.fontGreen = CreateFont("Match")
	esm.fontGreen:CopyFontObject("GameFontNormal");
	esm.fontGreen:SetTextColor(0, 1, 0, 1.0);
	esm.fontGreen:SetFont(fontStr, fontSize, "OUTLINE")
	
	esm.fontRed = CreateFont("Missing")
	esm.fontRed:CopyFontObject("GameFontNormal");
	esm.fontRed:SetTextColor(1, 0, 0, 1.0);
	esm.fontRed:SetFont(fontStr, fontSize, "OUTLINE")

end


esm.createScrollFrame = function()

	esm.buttonContainer = CreateFrame("Frame", "EssenceSetManagerBtnContainer", esm.scrollFrame)

	esm.buttonContainer:SetWidth(BUTTON_SIZE + 20)

	esm.scrollFrame = CreateFrame("ScrollFrame", nil, AzeriteEssenceUI, "UIPanelScrollFrameTemplate")
	esm.scrollFrame:SetPoint("TOPLEFT", AzeriteEssenceUI, "TOPLEFT", X_OFFSET, Y_OFFSET)
	esm.scrollFrame:SetSize(BUTTON_SIZE+20, AzeriteEssenceUI:GetHeight()+Y_OFFSET-15)
	esm.scrollFrame:SetScrollChild(esm.buttonContainer)
...
esm.createSaveFrames = function()
	...
	if ElvUI then --more room if you use ElvUI because the giant azerite icon isn't there
		...
		if E.private.skins.blizzard.AzeriteEssence then
			Y_OFFSET = -35
			MAX_SETS = 9
		end
	end
	
	if IsAddOnLoaded("AzeritePowerWeights") then
		Y_OFFSET = Y_OFFSET - 75
		SAVE_X_OFFSET = 90
		SAVE_Y_OFFSET = -435
		X_OFFSET = 0
		MAX_SETS = MAX_SETS - 2
		SCROLL_HEIGHT_OFFSET = 20
	end
	
	esm.input = CreateFrame("EditBox", "", AzeriteEssenceUI, "InputBoxTemplate");
	esm.input:SetPoint("TOPLEFT", AzeriteEssenceUI, "TOPLEFT" , 100 + SAVE_X_OFFSET, -35 + SAVE_Y_OFFSET)
	esm.input:SetFrameStrata("DIALOG")

	esm.input:SetSize(80,23)
	...
	esm.itemBtn:SetPoint("RIGHT", esm.save, "RIGHT" , 115, 0)
	esm.itemBtn:SetNormalFontObject("GameFontNormal")
	--esm.itemBtn:SetSize(50, 50)
	esm.itemBtn:SetWidth(110)
	esm.itemBtn:SetHeight(25)
...
esm.createSetButton = function(text, i)
	...
	if not setButtons[i] then
		setButtons[i] = CreateFrame("Button", "", esm.buttonContainer,  "UIPanelButtonTemplate")
		setButtons[i]:SetPoint("TOPLEFT", esm.buttonContainer, "TOPLEFT" , BUTTON_PAD+10, ((BUTTON_SIZE + (BUTTON_PAD)) * i * -1)- BUTTON_PAD)
		setButtons[i]:SetWidth(BUTTON_SIZE)
		setButtons[i]:SetHeight(BUTTON_SIZE)
...
esm.checkIfCanChange = function() --returns correct saturation depending if you can change essences or not
	if IsResting() then
		return nil
	end
	for i = 1, 40 do
